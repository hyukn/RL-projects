...
class PatchworkMMABPSolver:
    ...
    def alphabeta(self, state: PatchworkState, depth, alpha, beta, player, parallel):
        ...
        if state.current_player() == player:
            ...
            parallel_condition = parallel and depth > self.DEPTH_THRED and len(legal_actions) >= 3
                ...
                if i == 0:
                    ...
                    if parallel_condition:
                        ps.append([pool.apply_async(self.alphabeta,
                            args=(state_new, depth, alpha, beta, player, False)
                        ), action])
                    else:
                        eval, _ = self.alphabeta(state_new, depth, alpha, beta, player, parallel)
                        alpha = max(alpha, eval)
            ...
            if parallel_condition:
                results = [(p.get()[0], action) for p, action in ps] + [(max_eval, max_action)]
            ...
            for i, action in enumerate(legal_actions):
                ...
                if i == 0:
                    ...
                    if parallel_condition:
                        ps.append([pool.apply_async(self.alphabeta,
                            args=(state_new, depth_new, alpha, beta, player, False)
                        ), action])
                    else:
                        eval, _ = self.alphabeta(state_new, depth_new, alpha, beta, player, parallel)
                        beta = min(beta, eval)
            ...
            if parallel_condition:
                results = [(p.get()[0], action) for p, action in ps] + [(min_eval, 0)]
                min_eval, _ = min(results, key=lambda x: x[0])

            return min_eval, _

class AgentMMP(object):
    def __init__(self, depth, player):
        ...
        self.solver = PatchworkMMABPSolver(
            placer=self.placer, 
