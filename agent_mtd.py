...
class PatchworkMTDSolver:
    ...
    def ABMM_solve_prev(self, state: PatchworkState, depth, alpha, beta, player):
    ...
    def ABMM_solve(self, state: PatchworkState, depth, alpha, beta, player):
        if depth == 0 or state.is_terminal():
            # TODO: use more efficient returns to evaluate the current state
            # print(sd)
            avg = state.avg_returns(player)
        ...
        if state.current_player() == player:
            ...
            for action in legal_actions:
                ...
                # state_new = copy.copy(state)
                state_new.do_apply_action(action)
                eval, _ = self.ABMM_solve(state_new, depth, alpha, beta, player)
                alpha = max(alpha, eval)
            ...
            for action in legal_actions:
                ...
                state_new.do_apply_action(action)
                if state_new.current_player() == player:
                    eval, _ = self.ABMM_solve(state_new, depth - 1, alpha, beta, player)
                else:
                    eval, _ = self.ABMM_solve(state_new, depth, alpha, beta, player)
                beta = min(beta, eval)
                if eval < min_eval:
    ...
    def solve(self, state: PatchworkState, depth, player):
        self.history = set()

        lower, upper = -162, 200
        EVAL_ROUGH = 1
        # binary search
        while lower < upper - EVAL_ROUGH:
            gamma = (lower + upper) // 2
            score, action = self.ABMM_solve(state, depth, gamma - 1, gamma, player)
            if score >= gamma:
                lower = score
            if score < gamma:
                upper = gamma

        # make another call that produce a move
        # score, action = self.ABMM_solve(state, depth, lower, lower + 1, depth, player)
        return score, action

class AgentMTD(object):
    def __init__(self, depth, player):
        ...
        self.solver = PatchworkMTDSolver(placer=self.placer, evaluator=self.evaluator)
