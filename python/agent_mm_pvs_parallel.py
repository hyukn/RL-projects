...
class PatchworkMMABPVSSolver:
    ...
    def solve_multi(self, state: GameState, depth, alpha, beta, player):
        manager = mp.Manager()
        self.DEPTH_THRED = depth
        parallel = False
        max_eval, max_action = self.alphabeta(state, depth, alpha, beta, player, parallel)

        return max_eval, max_action

    def alphabeta(self, state: PatchworkState, depth, alpha, beta, player, parallel):
        self.node_visits += 1
        if depth == 0 or state.is_terminal():
            # print(f'branches: {self.branches}')
            avg = state.avg_returns(player)
            score = avg
            # adjust zero window searching
            score = int(score * 100)
            return score, None
        ...
        if state.current_player() == player:
            ...
            ps = list()
            parallel_condition = parallel and depth > 1 and len(legal_actions) >= 3
            if parallel_condition:
                pool = Pool(int(mp.cpu_count()))
            ...
            for i, action in enumerate(legal_actions):
                ...
                if i == 0:
                    ...
                    eval, _ = self.alphabeta(state_new, depth, alpha, beta, player, parallel)
                    ...
                    if parallel_condition:
                        state_new = copy.copy(state)
                        state_new.do_apply_action(action)
                        ps.append(pool.apply_async(self.alphabeta,
                            args=(state, depth, alpha, beta, player, False)
                        ))
                    else:
                        state_new = copy.copy(state)
                        state_new.do_apply_action(action)
                        # zero window search
                        eval, _ = self.alphabeta(state_new, depth, alpha, alpha + 1, player, False)
            ...
            if parallel_condition:
                results = [p.get() for p in ps] + [(max_eval, max_action)]
                max_eval, max_action = max(results, key=lambda x: x[0])
            ...
            for i, action in enumerate(legal_actions):
                ...
                if i == 0:
                    ...
                    eval, _ = self.alphabeta(state_new, depth_new, alpha, beta, player, parallel)
                    ...
                    if parallel_condition:
                        ...
                        depth_new = depth - 1 if state_new.current_player() == player else depth                        
                        ps.append(pool.apply_async(self.alphabeta,
                            args=(state, depth_new, alpha, beta, player, False)
                        ))
                    else:
                        # zero window search
                        eval, _ = self.alphabeta(state_new, depth_new, alpha, alpha + 1, player, False)
                        # failure on zero window search
                        # NOTE: remove
                        beta = min(beta, eval)
                        if eval < min_eval:
            ...
            if parallel_condition:
                results = [p.get() for p in ps] + [(min_eval, 0)]
                min_eval, _ = max(results, key=lambda x: x[0])

            return min_eval, _

class AgentMMPVSParallel(object):
    def __init__(self, depth, player):
        ...
        self.solver = PatchworkMMABPVSSolver(
            placer=self.placer, 
